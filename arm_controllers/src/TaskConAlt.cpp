...
{
class TaskConAlt : public controller_interface::Controller<hardware_interface::EffortJointInterface> //for commanding effort-based joints. 
{
  public:
    bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n)
    {
		...
		if (!n.getParam("/elfin/TaskConAlt/clik_gain/K_regulation", K_regulation_)) //Load Regulation gain
		{
        ...
        tau_d_ = Eigen::VectorXd::Zero(n_joints_);
        ...
        // 6.2 subsriber
        sub_x_cmd_ = n.subscribe<std_msgs::Float64MultiArray>("command", 1, &TaskConAlt::commandCB, this);
    ...
    void update(const ros::Time &time, const ros::Duration &period) //This is called periodically when the controller is running. 
    {
		...
		else
		{
            ...
            enforceJointLimits(qd_(0), 0);
            enforceJointLimits(qd_(1), 1);
            enforceJointLimits(qd_(2), 2);
            enforceJointLimits(qd_(3), 3);
            enforceJointLimits(qd_(4), 4);
            enforceJointLimits(qd_(5), 5);
...
**/
        ...
        {
            // 
            tau_d_(i) = C_(i) + G_(i); //+ tau_fric_(i);
            //controller_state_pub_->msg_.effort_feedforward[i] = tau_d_(i);
            tau_d_(i) += pids_[i].computeCommand(e_(i), e_dot_(i), period);

            // effort saturation
            if (tau_d_(i) >= joint_urdfs_[i]->limits->effort)
                tau_d_(i) = joint_urdfs_[i]->limits->effort;
  ...
  private:
    ...
    Eigen::VectorXd tau_d_, tau_fric_;
...
PLUGINLIB_EXPORT_CLASS(arm_controllers::TaskConAlt, controller_interface::ControllerBase)